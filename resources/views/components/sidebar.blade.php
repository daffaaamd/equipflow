...
<aside id="app-sidebar"
                ...
                <div>
                    ...
                    <ul class="space-y-0.5">
                        @foreach ($group['items'] as $item)
                            <li>
                                <a href="{{ $item['url'] }}"
                                   class="flex items-center gap-3 rounded-sm px-3 py-2 text-sm font-medium transition-colors {{ ($item['active'] ?? false) ? 'bg-brand-500 text-white' : 'text-charcoal-300 hover:bg-navy-900 hover:text-white' }}">
                                    <svg class="shrink-0" style="width:18px;height:18px" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.7">{!! $item['icon'] !!}</svg>
    ...
    <div class="border-t border-navy-800 p-4">
        <div class="flex items-center gap-3">
            <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-navy-800 font-display text-sm font-bold text-white">{{ strtoupper(substr(auth()->user()->name, 0, 2)) }}</span>
            <div class="min-w-0">
